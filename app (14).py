...
/* ================= Plain sky backdrop ================= */
/* Swapped to a plainer, lighter sky-only photo — "Minimal Nature" by Milad
   Fakurian on Unsplash (free to use, no attribution required:
   https://unsplash.com/photos/UiiHVEyxtyA) — since the earlier lake/mountain
   photo was too busy for text to sit on top of directly. Kept only a very
   light wash on top since this photo is already pale and even-toned. */
.stApp {
    background-image:
        linear-gradient(180deg, rgba(255,255,255,0.18) 0%, rgba(255,255,255,0.08) 50%, rgba(255,255,255,0.2) 100%),
        url('https://images.unsplash.com/photo-1596367407372-96cb88503db6?auto=format&fit=crop&w=1920&q=80');
...
@keyframes skyDrift {
    0%   { background-position: 50% 40%, 50% 40%; }
    50%  { background-position: 50% 40%, 52% 46%; }
    100% { background-position: 50% 40%, 50% 40%; }
}
@keyframes blobFloat {
